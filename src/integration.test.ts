...
function makeConfig(
  ...
  return {
    ...
    gateway: { maxQueueSize: 20, processingUpdateIntervalMs: 5000, workloadLockFiles: [], rateLimiter: { enabled: false, windowMs: 60_000, maxRequests: 20 } },
...
describe("Integration: Queue processes messages in order", () => {
  ...
  beforeEach(() => {
    ...
    config = makeConfig("/tmp/workspace", "/tmp/data", {
      gateway: { maxQueueSize: 10, processingUpdateIntervalMs: 5000, workloadLockFiles: [], rateLimiter: { enabled: false, windowMs: 60_000, maxRequests: 20 } },
    } as Partial<Config>);
  });
  ...
  it("rejects messages when queue is full", () => {
    const smallConfig = makeConfig("/tmp/workspace", "/tmp/data", {
      gateway: { maxQueueSize: 2, processingUpdateIntervalMs: 5000, workloadLockFiles: [], rateLimiter: { enabled: false, windowMs: 60_000, maxRequests: 20 } },
    } as Partial<Config>);
